...
const DailyBestSells = () => {
  ...
  return (
    <div className="padding-container max-container py-6 text-green-30 text-xs lg:text-sm font-[600]">
        ...
        {/* Product List */}
        <button type="button" onClick={prevProduct} className="p-3 bg-gray-90 rounded-full absolute mt-64 left-[360px]"><LongArrowIcon className="fill-gray-10" /></button>
        <button type="button" onClick={nextProduct} className="p-3 bg-gray-90 rounded-full rotate-180 absolute mt-64 right-28"><LongArrowIcon className="fill-gray-10" /></button>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 py-6">
            <div className="hidden md:flex flex-col rounded-2xl bg-green-10 bg-daily-best bg-cover bg-center px-10 py-20">
