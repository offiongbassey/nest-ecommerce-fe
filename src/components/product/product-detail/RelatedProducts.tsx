            ...
            {related_products.map((product, index) => {
                ...
                return (
                    // <Link  href={`/product/${slug}`}>
                        <ProductCard key={index} variant='#F74B81' slug={slug} product_images={product_images} type="Hot">
                            <div className='p-4'>
                                <h2 className='font-[600] text-green-30 text-sm mt-3'>{name}</h2>
                                <div className='flex items-center py-1'> {Array(4).fill(1).map((_, _index) => ( <StarIcon key={_index} className='fill-[#FEC601]'/>  ))}</div>
